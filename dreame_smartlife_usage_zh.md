# Dreame Smart Life 接入说明

我已经在工作区生成了一个 Home Assistant 自定义集成骨架：
...
2. 重启 HA。
3. 在 HA 里添加集成 `Dreame Smart Life`。
...
追觅空气净化器的云协议已经确认了，但净化器具体用哪个 `siid/piid` 表示：
...
这些点位，APK 里没有直接给出完整映射，所以我把“探测 + 手工映射”的能力先做进 HA。
...
先直接使用已经内置好的实体。如果你的机型后续还有额外功能点需要补，再根据实际属性继续补映射。
